...
- **[TypeScript](yobi_TypeScript.md)** — ファイル全体の言語
- **[Next.js](yobi_Nextjs.md)** — `Metadata` 型でSEO設定を記述する仕組みを提供
- **[JSX](yobi_React.md#jsxhtmlっぽい記法)** — `<html>` `<body>` などのHTML構造を書くための記法
- **[Tailwind CSS](yobi_TailwindCSS.md)** — `h-full` などのクラス名でスタイルを指定
...
[`robots: { index: false }`](yobi_Nextjs.md#robots-設定) はGoogleに「このサイトを検索結果に載せないで」と伝えています。
...
- **[TypeScript](yobi_TypeScript.md)** — ファイル全体の言語
- **[Next.js](yobi_Nextjs.md)** — URLにアクセスされたとき最初に実行されるファイルとして機能する
- **[JSX](yobi_React.md#jsxhtmlっぽい記法)** — `<AppShell />` のようなコンポーネントを書くための記法
...
- **[TypeScript](yobi_TypeScript.md)** — ファイル全体の言語。`type Screen = ...` で型を定義
- **[React（useState）](yobi_React.md#usestate状態管理)** — 現在の画面（`screen`）を状態として管理する
- **[React（useEffect）](yobi_React.md#useeffect副作用)** — 起動時に1回だけ「戻るボタン無効化」を実行する
- **[JSX](yobi_React.md#jsxhtmlっぽい記法)** — 画面コンポーネントを条件付きで表示する記述
- **[ブラウザ History API](yobi_BrowserAPI.md#history-api戻るボタン制御)** — `history.pushState` / `popstate` でブラウザの「戻る」を制御
...
- **[TypeScript](yobi_TypeScript.md)** — ファイル全体の言語
- **[React（useState）](yobi_React.md#usestate状態管理)** — モーダルの開閉状態（`open`）を管理する
- **[React（useEffect）](yobi_React.md#useeffect副作用)** — 起動時に「過去に見たか」を確認してモーダルを開くかどうか判断する
- **[Radix UI（Dialog）](yobi_RadixUI.md)** — アクセシビリティ対応済みのモーダルを提供する外部ライブラリ
- **[Lucide React](yobi_LucideReact.md)** — `ShieldCheck` `X` アイコンを表示するライブラリ
- **[Tailwind CSS](yobi_TailwindCSS.md)** — モーダルのレイアウト・色・角丸などのスタイル
- **[sessionStorage（Web API）](yobi_BrowserAPI.md#sessionstorage一時データ保存)** — 「このモーダルをすでに見たか」をブラウザに一時保存する
...
- **[TypeScript](yobi_TypeScript.md)** — ファイル全体の言語。`interface Props` でpropsの型を定義
- **[React（useState）](yobi_React.md#usestate状態管理)** — テキスト入力欄の内容（`input`）を管理する
- **[React（useRef）](yobi_React.md#userefdom要素への参照)** — テキスト入力欄のDOM要素に直接アクセスするために使用
- **[Lucide React](yobi_LucideReact.md)** — `Phone` `X` `Mic` `Shield` `HeartHandshake` アイコン
- **[Tailwind CSS](yobi_TailwindCSS.md)** — 画面全体のレイアウト・色・余白などのスタイル
- **[JSX](yobi_React.md#jsxhtmlっぽい記法)** — 画面のHTMLを書くための記法
- **[Web API（window.location）](yobi_BrowserAPI.md#windowlocationページ移動)** — クイック退出ボタンで別ページへ移動する処理
...
- **[TypeScript](yobi_TypeScript.md)** — ファイル全体の言語。`interface Props` でpropsの型を定義
- **[React（useState）](yobi_React.md#usestate状態管理)** — テキスト入力欄の内容（`input`）を管理する
- **[Lucide React](yobi_LucideReact.md)** — `Phone` `X` `Mic` アイコン
- **[Tailwind CSS](yobi_TailwindCSS.md)** — 4列グリッド・カードの角丸・アニメーションなどのスタイル
- **[JSX](yobi_React.md#jsxhtmlっぽい記法)** — カードを `.map()` で自動生成するための記述
- **[Web API（window.location）](yobi_BrowserAPI.md#windowlocationページ移動)** — クイック退出ボタンの処理
...
- **[TypeScript](yobi_TypeScript.md)** — ファイル全体の言語
- **[React（useEffect）](yobi_React.md#useeffect副作用)** — 起動時に「戻るボタン無効化」を1回実行する
- **[Lucide React](yobi_LucideReact.md)** — `LogOut` アイコン
- **[Tailwind CSS](yobi_TailwindCSS.md)** — ボタンの見た目（丸・赤・固定位置）のスタイル
- **[ブラウザ History API](yobi_BrowserAPI.md#history-api戻るボタン制御)** — `history.pushState` / `popstate` でブラウザの「戻る」を制御
- **[Web API（window.location）](yobi_BrowserAPI.md#windowlocationページ移動)** — `location.replace()` でクイック退出を実行
...
- [ ] [`useState`](yobi_React.md#usestate状態管理) は値が変わると画面が更新される変数である
- [ ] [`props`](yobi_React.md#propsコンポーネントへの引数) はコンポーネントへの引数である
- [ ] [`useEffect`](yobi_React.md#useeffect副作用) の `[]` は「最初の1回だけ実行」という意味
- [ ] [`sessionStorage`](yobi_BrowserAPI.md#sessionstorage一時データ保存) はタブを閉じると消える（`localStorage` との違い）
- [ ] [`window.location.replace()`](yobi_BrowserAPI.md#windowlocationページ移動) は遷移後に「戻る」で戻れない
- [ ] [`history.pushState`](yobi_BrowserAPI.md#history-api戻るボタン制御) は「戻る」ボタンを無効化するために使っている
- [ ] [`robots: { index: false }`](yobi_Nextjs.md#robots-設定) はGoogleの検索結果に出ないようにする設定
- [ ] [`.map()`](yobi_React.md#mapリストのレンダリング) は配列の全要素に同じ処理をする（forループの代わり）
